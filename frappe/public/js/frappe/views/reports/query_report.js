...
frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
	...
	pdf_report(print_settings) {
		...
		for (let key of Object.keys(applied_filters)) {
			...
			if (!filter_df || filter_df.fieldtype == "Check" || filter_df.hidden || filter_df.read_only) {
				continue;
			}

			let current_filter_value_str = applied_filters[key].toString();
			if (!current_filter_value_str) {
				continue;
			}

			name_len = name_len + current_filter_value_str.length;
			if (name_len > 200) {
				break;
			}
	...
	export_report() {
		...
		this.export_dialog = frappe.report_utils.get_export_dialog(
			...
			({
				...
				if (visible_idx.length + 1 === data?.length) {
	...
	show_footer_message() {
		...
		this.$report_footer = $(`<div class="report-footer text-muted">
			<div class="report-footer-items"></div>
		...
		const execution_time_msg = __("Execution Time: {0} sec", [flt(this.execution_time, 3) || 0.1]);
