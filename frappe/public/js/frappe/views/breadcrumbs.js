...
frappe.breadcrumbs = {
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		if (
			...
			if (doctype_meta?.default_view || frappe.model.user_settings[doctype]?.last_view) {
				let view = doctype_meta?.default_view || frappe.model.user_settings[doctype].last_view;
