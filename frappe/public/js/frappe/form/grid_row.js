	...
	add_open_form_button() {
		...
		if (this.doc && !this.grid.df.in_place_edit) {
			...
			if (!this.open_form_button) {
				this.open_form_button = $('<div class="row-edit col"></div>').appendTo(this.row);
	...
	add_column_configure_button() {
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="row-edit col grid-static-col d-flex justify-content-center" style="cursor: pointer;">
			...
			this.configure_columns_button = $(`
				<div class="row-edit col grid-static-col"></div>
			`).appendTo(this.row);
		}
	}
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $(
			...
			.on("click", function (event) {
				...
				var col = this;
				let first_input_field = $(col).find('input:first, textarea:first, select:first');
				let input_in_focus = false;

				$(col)
					.find("input:first, textarea:first, select:first")
					.each(function () {
						if ($(this).is(":focus")) {
							input_in_focus = true;
						}
