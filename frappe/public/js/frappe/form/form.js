...
frappe.ui.form.Form = class FrappeForm {
	...
	focus_on_first_input() {
		...
		let first = layout_wrapper.find(":input:visible:first");
		if (!["Date", "Datetime"].includes(first.attr("data-fieldtype"))) {
			first.trigger("focus");
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		let to_set = frappe.get_meta(doctype).fields.filter(df => {
			return (
				df.fieldtype === 'Link'
				&& df.options === me.doctype
				&& !df.hidden
				&& !df.read_only
			);
		});
