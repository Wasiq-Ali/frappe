...
frappe.ui.form.ControlLink = class ControlLink extends frappe.ui.form.ControlData {
	...
	setup_awesomeplete() {
		...
		this.$input.on(
			...
			frappe.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(frappe.boot.sysdefaults?.link_field_results_limit) || 20,
				};
		...
		this.$input.on("blur", function () {
			...
			let value = cstr(me.get_input_value());
			let label = cstr(me.get_label_value());
			let last_value = cstr(me.last_value);
			let last_label = cstr(me.label);
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let value = filter[3] == null || filter[3] === "" ? __("empty") : cstr(filter[3]);
