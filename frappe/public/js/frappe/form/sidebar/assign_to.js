...
frappe.ui.form.AssignToDialog = class AssignToDialog {
	...
	set_description_from_doc() {
		...
		let title = this.get_title_from_doc();
		if (title) {
			title = me.frm.doc.name + ": " + title;
		} else {
			title = me.frm.doc.name
		}

		me.dialog.set_value("title", title);
	}
	get_title_from_doc() {
		let me = this;

		if (me.frm && me.frm.meta.title_field) {
			return me.frm.doc[me.frm.meta.title_field];
		} else {
			return "";
		}
	}
