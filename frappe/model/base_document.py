...
def import_controller(doctype):
	...
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = frappe.db.get_value("DocType", doctype, ("module", "custom", "is_tree"), as_dict=True, cache=True)
		if doctype_info:
...
class BaseDocument:
	...
	def update_if_missing(self, d, force_fields=None):
		...
		for key, value in d.items():
			if (force_fields and key in force_fields) or (
				value is not None
	...
	def get_formatted(
		self, fieldname, doc=None, currency=None, absolute_value=False, translated=False, format=None, precision=None
	):
		...
		return format_value(val, df=df, doc=doc, currency=currency, format=format, precision=precision)
