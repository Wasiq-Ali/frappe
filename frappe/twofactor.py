...
def two_factor_is_enabled_for_(user):
	...
	roles = user_roles + [ALL_USER_ROLE]

	role_doctype = frappe.qb.DocType("Role")

	bypassed_roles = 0
	if user_roles:
		bypassed_roles = frappe.db.count(
			role_doctype,
			filters=((role_doctype.bypass_two_factor_auth == 1) & (role_doctype.name.isin(user_roles))),
		)

	if int(bypassed_roles) > 0:
		return False

	enabled_roles = frappe.db.count(
		role_doctype,
		filters=((role_doctype.two_factor_auth == 1) & (role_doctype.name.isin(roles))),
	)

	if int(enabled_roles) > 0:
		return True
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from frappe.core.doctype.sms_settings.sms_settings import send_via_gateway, clean_receiver_number
	except Exception:
		return False

	phone_no = clean_receiver_number(phone_no, format_sms=True)
	if not phone_no:
		return False

	ss = frappe.get_cached_doc("SMS Settings", "SMS Settings")
