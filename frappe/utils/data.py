...
def get_user_time_format() -> str:
	...
	if frappe.local.user_time_format == "12 Hour":
		return "hh:mm:ss a"
	else:
		return "HH:mm:ss"


get_time_format = get_user_time_format  # For backwards compatibility
...
def get_link_to_form(doctype: str, name: str, label: str | None = None, target: str | None = None) -> str:
	if not label:
		label = name

	target_str = ' target="{0}"'.format(target) if target else ""

	return f"""<a href="{get_url_to_form(doctype, name)}"{target_str}>{label}</a>"""
