...
def sync_fixtures(app=None, force=False):
	...
	for app in apps:
		import_fixtures(app, force=force)
		import_custom_scripts(app)

	frappe.flags.in_fixtures = False


def import_fixtures(app, force=False):
	...
	for fname in fixture_files:
		...
		try:
			import_doc(file_path, force=force)
...
def export_fixtures(app=None):
	...
	for app in apps:
		for fixture in frappe.get_hooks("fixtures", app_name=app):
			...
			meta = frappe.get_meta(fixture)
			order_by = "idx, creation" if meta.istable else "creation"
